...
struct CanonicalSyncKernelCompletionTests {
    ...
    @Test func audioUploadV855ReadinessScorecardIsReportOnlyUntilDeviceEvidenceOnMac() {
    ...
    @Test func finalGateV868ReadyForRealDeviceCanonicalSwitchWithoutDeviceEvidenceOnMac() {
    ...
    @Test func finalGateV868PartialWhenT1ResidualMissingButNoUnsafeBypassOnMac() {
        let scorecard = CanonicalSyncKernelCompletionScorecard.v868(
    ...
    @Test func finalGateV868NotReadyWhenBuildOrT2ToT6MissingOnMac() {
        let buildMissing = CanonicalSyncKernelCompletionScorecard.v868(macOSBuildPassed: false)
        let t4Missing = CanonicalSyncKernelCompletionScorecard.v868(
            t4ExecutorPortInjectionComplete: false
        )

        #expect(buildMissing.codeCompletionResult == .notReady)
        #expect(buildMissing.blockers.contains(.macOSBuildValidationMissing))
        #expect(t4Missing.codeCompletionResult == .notReady)
        #expect(t4Missing.blockers.contains(.t4ExecutorPortInjectionMissing))
    }

    @Test func finalGateV868UnsafeForReleaseCanonicalLegacyLossAndBypassOnMac() {
        let releaseCanonical = CanonicalSyncKernelCompletionScorecard.v868(
            releaseDefaultOldKernel: false
        )
        let legacyLoss = CanonicalSyncKernelCompletionScorecard.v868(
            legacyFallbackRetained: false
        )
        let routeChange = CanonicalSyncKernelCompletionScorecard.v868(
        ...
        #expect(legacyLoss.codeCompletionResult == .unsafeToTryOnDevice)
        #expect(legacyLoss.blockers.contains(.legacyFallbackUnavailable))
        #expect(routeChange.codeCompletionResult == .unsafeToTryOnDevice)
        #expect(routeChange.blockers.contains(.routeSecurityChanged))
    }

    @Test func appStateGateV873ReadyForRealDeviceAppTrialTracksMissingRealDeviceEvidenceOnMac() {
    ...
    @Test func appStateGateV873PartialWhenConvergenceRepairIsMissingOnMac() {
        let gate = CanonicalRealDeviceTrialReadinessGate.v873(
            statusConvergenceRefreshReady: false
        )

        #expect(gate.codeCompleteResult == .partialWithBlockers)
        #expect(gate.readyForRealDeviceAppTrial == false)
        #expect(gate.blockers.contains(.statusConvergenceRefreshMissing))
    }

    @Test func appStateGateV873NotReadyWhenBuildOrTargetedTestsMissingOnMac() {
        ...
        #expect(buildMissing.blockers.contains(.macOSBuildMissing))
        #expect(testsMissing.codeCompleteResult == .notReady)
        #expect(testsMissing.blockers.contains(.targetedTestsMissing))
    }

    @Test func appStateGateV873UnsafeForSecurityOrTopologyRegressionOnMac() {
        let requestVerifierBypassed = CanonicalRealDeviceTrialReadinessGate.v873(
            requestVerifierUnchanged: false
        )
        let reverseConnection = CanonicalRealDeviceTrialReadinessGate.v873(
            macDoesNotReverseConnectToIPhone: false
        )
        let retryStorm = CanonicalRealDeviceTrialReadinessGate.v873(
            retryStormProtectionReady: false
        )

        #expect(requestVerifierBypassed.codeCompleteResult == .unsafeToTryOnDevice)
        #expect(requestVerifierBypassed.blockers.contains(.requestVerifierBypassed))
        ...
        #expect(retryStorm.codeCompleteResult == .unsafeToTryOnDevice)
        #expect(retryStorm.blockers.contains(.retryStormProtectionMissing))
    }

    @Test func manualSwitchGateV868AllowsOnlyReadyScorecardWithOwnerBackupAndProofOnMac() {
    ...
    @Test func manualSwitchGateV868BlocksWithoutBackupSwitchBackOrSafeScorecardOnMac() {
        var harness = CanonicalLegacySwitchBackHarness()
    ...
    @Test func appStateGateV915ReadyForFourDomainTrialAllowsMissingRealDeviceEvidenceOnMac() {
    ...
    @Test func appStateGateV915NotReadyWhenAnyR1ThroughR7OrBuildSummaryIsMissingOnMac() {
    ...
    @Test func appStateGateV915UnsafeForProductionSafetyRegressionsOnMac() {
