...
actor CanonicalTestProductionFilePort: CanonicalProductionFilePort {
    ...
    func metadataSnapshot(objectID: String) async throws -> Data? { nil }
    func artifactDescriptor(for artifact: CanonicalArtifact) async throws -> CanonicalProductionArtifactDescriptor { CanonicalProductionArtifactDescriptor(artifact: artifact) }
    ...
    func projectWrite(_ intent: CanonicalFileWriteIntent) async throws -> CanonicalProductionWriteIntentProjection {
        CanonicalProductionWriteIntentProjection(reference: intent.reference, purpose: intent.purpose, wouldWrite: true, suppressedBecauseDryRun: false, noPhysicalDelete: true, byteSize: Int64(intent.bytes.count), contentHash: InMemoryCanonicalFileStore.hash(intent.bytes, policy: intent.hashPolicy), disposition: .created, reason: "testProductionProjection")
    }
    func readMetadata(_ request: CanonicalProductionMetadataReadRequest) async throws -> CanonicalProductionFileReadResult {
        ...
        return CanonicalProductionFileReadResult(bytes: read.bytes, purpose: read.purpose, evidence: evidence(reference: request.reference, resolution: read.resolution, expectedHash: nil, actualHash: read.contentHash, expectedByteSize: nil, actualByteSize: read.byteSize), metadataBlob: read.metadataBlob, tombstoned: read.tombstoned)
    }
    func writeMetadata(_ intent: CanonicalFileWriteIntent, rollbackCheckpoint: CanonicalRollbackCheckpoint?) async throws -> CanonicalProductionFileWriteResult {
        ...
        return CanonicalProductionFileWriteResult(disposition: write.disposition, purpose: write.purpose, evidence: evidence(reference: intent.reference, resolution: write.resolution, expectedHash: intent.expectedContentHash, actualHash: write.contentHash, expectedByteSize: intent.expectedByteSize, actualByteSize: write.byteSize), rollbackCheckpointID: rollbackCheckpoint?.checkpointID, tombstoned: write.tombstoned)
    }
    func readArtifact(_ request: CanonicalProductionArtifactReadRequest) async throws -> CanonicalProductionFileReadResult {
        let read = try await store.read(CanonicalFileReadRequest(reference: request.reference))
        return CanonicalProductionFileReadResult(bytes: read.bytes, purpose: read.purpose, evidence: evidence(reference: request.reference, resolution: read.resolution, expectedHash: request.expectedContentHash, actualHash: read.contentHash, expectedByteSize: request.expectedByteSize, actualByteSize: read.byteSize), metadataBlob: read.metadataBlob, tombstoned: read.tombstoned)
    }
    ...
    private func evidence(reference: CanonicalFileReference, resolution: CanonicalPathResolutionResult, expectedHash: CanonicalHash?, actualHash: CanonicalHash?, expectedByteSize: Int64?, actualByteSize: Int64?, streaming: Bool = false) -> CanonicalProductionFileVerificationEvidence {
        CanonicalProductionFileVerificationEvidence(reference: reference, resolution: resolution, expectedHash: expectedHash, actualHash: actualHash, expectedByteSize: expectedByteSize, actualByteSize: actualByteSize, computedStreaming: streaming)
    }
}
...
struct CanonicalTestProductionTransportPort: CanonicalProductionTransportPort {
    ...
    let routeCapabilities = CanonicalTransportRoute.allCases.map { CanonicalProductionTransportRouteCapability(route: $0, dryRunOnly: false) }
    ...
    func decodeResponseDryRun(_ response: CanonicalTransportResponse) async throws -> CanonicalTransportResponse { response }
    ...
    func sendRequest(_ request: CanonicalProductionSignedRequest) async throws -> CanonicalProductionTransportExchangeResult {
        CanonicalProductionTransportExchangeResult(request: request, response: CanonicalTransportResponse(ok: true, status: "ok", body: Data("{}".utf8)), responseVerified: true, usedExistingRoute: true, sideEffect: CanonicalProductionSideEffect(kind: .networkRequest, domain: .transportRuntime, route: request.buildRequest.route, summary: "testNetworkRequest"))
    }
}
...
actor CanonicalTestProductionUploadPort: CanonicalProductionUploadPort {
    ...
    func writeUploadLedger(_ snapshot: CanonicalProductionUploadLedgerSnapshot) async throws -> CanonicalProductionUploadLedgerSnapshot { snapshot }
    private func status(totalBytes: Int64?, hash: CanonicalHash, completed: Bool) -> CanonicalUploadSessionStatus {
        CanonicalUploadSessionStatus(ok: true, disposition: completed ? .finalized : .acceptedNew, phase: completed ? .completed : .active, sessionID: sessionID, confirmedBytes: confirmedBytes, nextOffset: confirmedBytes, chunkSize: chunkSizePolicy, completed: completed, finalFile: nil, checksum: completed ? hash : nil, fileSize: completed ? totalBytes : nil, retry: nil, error: nil)
    }
}
...
struct CanonicalTestProductionApplyPort: CanonicalProductionApplyPort {
    ...
    private func result(request: CanonicalProductionApplyExecutionRequest, status: CanonicalApplyExecutionStatus, kind: CanonicalProductionSideEffectKind) -> CanonicalProductionApplyResult {
        CanonicalProductionApplyResult(actionID: request.action.actionID, status: status, precondition: CanonicalProductionApplyPrecondition(actionID: request.action.actionID, target: request.action.target, accepted: true), postcondition: CanonicalProductionApplyPostcondition(actionID: request.action.actionID, target: request.action.target, accepted: true), sideEffect: CanonicalProductionSideEffect(kind: kind, domain: .apply, objectID: request.action.target.objectID, summary: kind.rawValue), rollbackCheckpointID: request.rollbackCheckpointID)
    }
}
