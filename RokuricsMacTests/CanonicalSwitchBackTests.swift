...
struct CanonicalSwitchBackTests {
    @Test func rootSafetyRejectsProductionLikeRootsAndAcceptsTempCloneOnMac() throws {
    ...
    @Test func realisticFixtureAndCloneContainRequiredComponentsOnMac() throws {
        let fileManager = FileManager()
    ...
    @Test func domainSwitchBackMatrixCoversAllFiveBusinessDomainsOnMac() {
    ...
    @Test func kernelSwitchBackProofCoversOldNewOldNewSequenceAndEvidenceOnMac() throws {
        let fileManager = FileManager()
    ...
    @Test func v857ScorecardRequiresRealDeviceEvidenceAfterCodeProofPassesOnMac() throws {
        let fileManager = FileManager()
        let root = fileManager.temporaryDirectory
    ...
    @Test func debugSwitchBackDriverRejectsProductionCloneRootOnMac() throws {
        let fileManager = FileManager()
        ...
        let productionCloneRoot = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent("Library", isDirectory: true)
            .appendingPathComponent("Application Support", isDirectory: true)
            .appendingPathComponent("RokuricsMacLocal", isDirectory: true)
    ...
    @Test func debugSwitchBackDriverRunsTempCloneRoundtripAndExportsRedactedEvidenceOnMac() throws {
        let fileManager = FileManager()
        let sourceRoot = try makeDriverSourceRoot(fileManager: fileManager)
        let cloneRoot = fileManager.temporaryDirectory
    ...
    @Test func debugRunnerWritesRedactedJSONLEvidenceAndUISafeSummaryOnMac() throws {
        let fileManager = FileManager()
        let sourceRoot = try makeDriverSourceRoot(fileManager: fileManager)
        ...
        #expect(text.contains("\"nodeRole\":\"mac\""))
    ...
    @Test func debugRunnerSurfacesBlockerForMissingSourceRootWithoutCrashOnMac() throws {
        let fileManager = FileManager()
    ...
    @Test func macDebugDriverAndSettingsButtonAreWiredUnderDebug() throws {
        _ = MacCanonicalSwitchBackProofDriver(fileManager: .default)

        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let settingsURL = repoRoot.appendingPathComponent("RokuricsMac/MacSettingsView.swift", isDirectory: false)
        ...
        #expect(settingsText.contains("MacCanonicalSwitchBackProofDriver().run()"))
        #expect(settingsText.contains("不直接写当前生产库"))
        #expect(settingsText.contains("不重启 receiver"))
        #expect(settingsText.contains("不触发 transcription/note"))
    }

    @Test func v862ScorecardBlocksManualSwitchWhenDriverProofIsMissingOnMac() {
