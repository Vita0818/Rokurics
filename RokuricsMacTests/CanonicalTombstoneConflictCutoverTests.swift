...
struct CanonicalTombstoneConflictCutoverTests {
    ...
    @Test func candidateGenerationCoversTombstoneConflictAndUnsupportedArtifactMarker() throws {
        ...
        let candidates = CanonicalTombstoneConflictCandidate.candidates(
            ...
            peerManifest: TombstoneConflictCutoverTestSupport.emptyManifest(nodeID: "iphone-01", platform: "iPhone")
        )
    ...
    @Test func noCommitExecutorStagesSummaryOnlyAndSuppressesProductionPaths() throws {
        let rootURL = TombstoneConflictCutoverTestSupport.makeScratchRoot("MacTombstoneConflictNoCommit")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let candidate = TombstoneConflictCutoverTestSupport.objectTombstoneCandidate().candidate
        let executor = MacTombstoneConflictNoCommitExecutor(stagingRootURL: rootURL)
    ...
    @Test func rootBoundCommitAndRollbackOnlyMutateMarkerOrLedger() async throws {
        let rootURL = TombstoneConflictCutoverTestSupport.makeScratchRoot("MacTombstoneConflictRootBound")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let candidate = TombstoneConflictCutoverTestSupport.objectTombstoneCandidate().candidate
    ...
    @Test func productionRootApplyPortIsDisabledByDefault() async throws {
        let rootURL = TombstoneConflictCutoverTestSupport.makeScratchRoot("MacTombstoneConflictProductionDisabled")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let candidate = TombstoneConflictCutoverTestSupport.objectTombstoneCandidate().candidate
        let applyPort = try MacTombstoneConflictRealApplyPort(productionRootURL: rootURL)
        let executor = MacTombstoneConflictCutoverExecutor(applyPort: applyPort)
