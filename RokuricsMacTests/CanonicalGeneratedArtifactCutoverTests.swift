...
struct CanonicalGeneratedArtifactCutoverTests {
    ...
    @Test func noCommitExecutorStagesOnlyRedactedSummaryAndPreservesLegacyFallback() throws {
        let rootURL = GeneratedArtifactCutoverTestSupport.makeScratchRoot("MacGeneratedArtifactNoCommit")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let candidate = GeneratedArtifactCutoverTestSupport.candidate()
        let executor = MacGeneratedArtifactNoCommitExecutor(stagingRootURL: rootURL)
    ...
    @Test func rootBoundApplyCommitAndRollbackRestorePreviousBytes() async throws {
        let rootURL = GeneratedArtifactCutoverTestSupport.makeScratchRoot("MacGeneratedArtifactRootBound")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        ...
        let applyPort = try MacGeneratedArtifactRealApplyPort(testRootURL: rootURL)
        try await GeneratedArtifactCutoverTestSupport.seedPayload(candidate, into: applyPort)
        let executor = MacGeneratedArtifactCutoverExecutor(
            applyPort: applyPort,
            peerNode: GeneratedArtifactCutoverTestSupport.macNode()
        )
    ...
    @Test func productionRootApplyPortIsDisabledByDefault() async throws {
        let rootURL = GeneratedArtifactCutoverTestSupport.makeScratchRoot("MacGeneratedArtifactProductionDisabled")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let candidate = GeneratedArtifactCutoverTestSupport.candidate()
        let applyPort = try MacGeneratedArtifactRealApplyPort(productionRootURL: rootURL)
        try await GeneratedArtifactCutoverTestSupport.seedPayload(candidate, into: applyPort)
        let executor = MacGeneratedArtifactCutoverExecutor(
            applyPort: applyPort,
            peerNode: GeneratedArtifactCutoverTestSupport.macNode()
...
enum GeneratedArtifactCutoverTestSupport {
    ...
    static func seedPayload(
        ...
        into applyPort: MacGeneratedArtifactRealApplyPort
    ) async throws {
