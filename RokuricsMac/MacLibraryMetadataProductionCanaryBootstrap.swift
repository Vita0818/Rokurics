...
struct MacLibraryMetadataProductionCanaryBootstrapResult {
    var configuration: CanonicalLibraryMetadataProductionCanaryConfiguration
    var applyPort: MacLibraryMetadataRealApplyPort?
...
struct MacLibraryMetadataProductionCanaryBootstrap {
    var configuration: CanonicalLibraryMetadataProductionCanaryConfiguration
    ...
    func prepare(
    ...
    ) throws -> MacLibraryMetadataProductionCanaryBootstrapResult {
        guard configuration.mode != .disabled else {
            return MacLibraryMetadataProductionCanaryBootstrapResult(
        ...
        let applyPort: MacLibraryMetadataRealApplyPort?
        var blockers: [CanonicalLibraryMetadataRealCanaryBlocker] = []
        ...
        case .testRoot:
            ...
            let port = try MacLibraryMetadataRealApplyPort(
        ...
        let executor = applyPort.map {
            MacLibraryMetadataCutoverExecutor(applyPort: $0, failureInjection: failureInjection)
        }
        return MacLibraryMetadataProductionCanaryBootstrapResult(
            configuration: configuration,
            applyPort: applyPort,
