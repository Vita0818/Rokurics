...
struct MacTombstoneConflictReadSideSeam {
    ...
    private func legacySnapshot(
        ...
        let peerFacts = peerInventory.map { legacyFacts(from: $0, peer: true) } ?? []
        return CanonicalTombstoneConflictReadProjection.snapshot(
            source: .legacy,
            facts: legacyFacts(from: localInventory, peer: false) + peerFacts
        )
    }
