...
struct MacLibraryMetadataNoCommitExecutor: CanonicalLibraryMetadataNoCommitExecutor {
    ...
    private nonisolated func stage(
        ...
        guard candidate.cutoverCandidate.cutoverActionKind.isExecutableMetadata else {
            return CanonicalLibraryMetadataNoCommitStagingResult(
                candidate: candidate,
                staged: false,
                wroteOnlyStagingRoot: true,
                stagingEvidence: lifecycle.stagingEvidence(status: .notCreated),
                failure: .unsupportedAction,
                reason: "unsupportedLibraryMetadataNoCommitAction"
            )
        }
        ...
        let summary = CanonicalLibraryMetadataNoCommitPayloadSummary(candidate: candidate)
        let bytes = summary.encodedBytes()
        let hash = CanonicalTransportEnvelope.hash(bytes)
        let logicalPathToken = "library-metadata/\(platformPrefix)-\(safePathComponent(candidate.cutoverCandidate.objectID))-\(safePathComponent(candidate.id)).json"
        guard let safeLogicalPathToken = CanonicalProjectionContract.safeLogicalPathToken(logicalPathToken) else {
            ...
            return CanonicalLibraryMetadataNoCommitStagingResult(
                ...
                cleanupEvidence: CanonicalNoCommitCleanupEvidence(result: cleanup),
                failure: .stagingFailed,
                reason: "unsafeLibraryMetadataNoCommitLogicalPath"
            )
        }
        ...
        guard destinationURL.path.hasPrefix(rootPath) else {
            ...
            return CanonicalLibraryMetadataNoCommitStagingResult(
                ...
                cleanupEvidence: CanonicalNoCommitCleanupEvidence(result: cleanup),
                failure: .stagingFailed,
                reason: "libraryMetadataStagingDestinationEscapedRoot"
            )
        }
        ...
        do {
            ...
            return CanonicalLibraryMetadataNoCommitStagingResult(
                ...
                cleanupEvidence: CanonicalNoCommitCleanupEvidence(result: cleanup),
                reason: "macLibraryMetadataNoCommitWouldUseMetadataManifestBridge"
            )
        } catch {
            ...
            return CanonicalLibraryMetadataNoCommitStagingResult(
                ...
                cleanupEvidence: CanonicalNoCommitCleanupEvidence(result: cleanup),
                failure: .stagingFailed,
                reason: "macLibraryMetadataNoCommitStagingFailed"
            )
        }
    }

    private nonisolated func makeLifecycle(platformPrefix: String) -> CanonicalNoCommitStagingRootLifecycle {
        let rootID = "\(platformPrefix)-library-metadata-\(UUID().uuidString)"
        let rootURL = stagingRootURL ?? FileManager.default.temporaryDirectory
            .appendingPathComponent("RokuricsCanonicalV810LibraryMetadataNoCommit", isDirectory: true)
