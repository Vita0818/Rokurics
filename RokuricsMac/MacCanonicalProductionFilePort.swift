...
actor MacCanonicalProductionFilePort: CanonicalProductionFilePort {
    ...
    init(testRootURL: URL, rootToken: CanonicalRootToken = CanonicalRootToken("mac-test-root"), fileManager: FileManager = .default) {
    ...
    init(
        ...
        rootToken: CanonicalRootToken = CanonicalRootToken("mac-production-root"),
    ...
    private func write(
        ...
        guard !isDryRunOnly else {
            throw CanonicalProductionPortError.productionMutationAttempted("macProductionFilePortDisabled")
        }
    ...
    private func read(
        ...
        guard !isDryRunOnly else {
            throw CanonicalProductionPortError.productionMutationAttempted("macProductionFilePortReadDisabled")
        }
        let resolved = try resolvedURL(for: reference)
        ...
        return CanonicalProductionFileReadResult(
            ...
            metadataBlob: purpose == .metadataBlob ? CanonicalMetadataBlob(["source": "macProductionFilePort"]) : nil,
    ...
    private func resolvedURL(for reference: CanonicalFileReference) throws -> (url: URL, resolution: CanonicalPathResolutionResult) {
        ...
        guard isInsideRoot(url, root: root) else {
            throw CanonicalProductionPortError.pathEscapeRisk("macRootEscape")
        }
        return (
            url,
    ...
    private func rootURL(for rootToken: CanonicalRootToken) throws -> URL {
        ...
        case let .blocked(_, gate):
            throw CanonicalProductionPortError.productionMutationAttempted("macProductionFilePortGateBlocked:\(gate.blockers.map(\.rawValue).joined(separator: ","))")
        case .disabled:
            throw CanonicalProductionPortError.productionMutationAttempted("macProductionFilePortDisabled")
        }
    ...
    private var projectionReason: String {
        if isDryRunOnly {
            if let gate = productionRootGate, !gate.allowed {
                return "macProductionFilePortGateBlocked:\(gate.blockers.map(\.rawValue).joined(separator: ","))"
            }
            return "macProductionFilePortDisabled"
        }
        switch mode {
        case .testRoot(_, _):
            return "macTestRootWriteAvailable"
        case .productionRoot(_, _, _):
            return "macProductionRootGatedWriteAvailable"
        case .disabled, .blocked(_, _):
            return "macProductionFilePortDisabled"
        }
    }
