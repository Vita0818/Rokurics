...
nonisolated struct MacCanonicalShadowPortFactory: Sendable {
    ...
    nonisolated func makeOutput(
        ...
        let localSnapshot = Self.snapshot(
            ...
            action: "MacShadowPortFactoryLocal",
        ...
        let peerSnapshot = peerInventory.flatMap {
            Self.snapshot(
                from: $0,
                legacyActions: .empty,
                action: "MacShadowPortFactoryPeer",
                generatedAt: generatedAt
            )
        }
    ...
    nonisolated private static func makePortSet(for configuration: CanonicalShadowMigrationConfiguration) -> (portSet: CanonicalProductionPortSet, lifecycle: CanonicalShadowRootLifecycle?) {
        guard configuration.effectiveMode.runsExecutionShadowPreparation else {
            return (MacCanonicalDryRunPorts.makePortSet(), nil)
        }
        ...
        let rootToken = CanonicalRootToken("mac-execution-shadow-root")
        let binding = CanonicalShadowRootBinding(
            rootToken: rootToken,
            rootKind: rootKind,
            rootURL: rootURL
        )
        guard let filePort = try? MacCanonicalShadowFilePort(binding: binding) else {
            return (MacCanonicalDryRunPorts.makePortSet(), nil)
        }
        ...
        return (CanonicalProductionPortSet(
            file: filePort,
            transport: MacCanonicalShadowTransportPort(),
