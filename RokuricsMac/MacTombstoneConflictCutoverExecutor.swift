...
actor MacTombstoneConflictCutoverExecutor: CanonicalTombstoneConflictCutoverExecutor {
    ...
    init(
        applyPort: any CanonicalProductionApplyPort = MacTombstoneConflictRealApplyPort(),
    ...
    func commitTombstoneConflict(
        ...
        guard !applyPort.isDryRunOnly else {
            return .failure(candidate: candidate, kind: .applyFailureBeforeCommit, reason: "macTombstoneConflictCommitRequiresExplicitTestRootApplyPort")
        }
        ...
        do {
            ...
            return .failure(candidate: candidate, kind: .rootBoundWriteUnavailable, reason: "macTombstoneConflictPayloadPreparationFailed")
        }
        ...
        do {
            ...
            return .failure(
                ...
                reason: "macTombstoneConflictApplyPortFailed"
            )
        }
    ...
    func rollbackTombstoneConflict(
        ...
        if reason == .applyFailureBeforeCommit || applyPort.isDryRunOnly {
            ...
            return CanonicalTombstoneConflictRollbackExecutionResult(
                checkpointID: candidate.effectiveRollbackCheckpointID,
                succeeded: true,
                reason: "macTombstoneConflictRollbackNoOp",
        ...
        guard checkpointIDs.contains(candidate.effectiveRollbackCheckpointID) else {
            ...
            return CanonicalTombstoneConflictRollbackExecutionResult(
                ...
                succeeded: false,
                fatal: true,
                reason: "macTombstoneConflictRollbackCheckpointMissing"
            )
        }
        let action = CanonicalRollbackAction(
            actionID: "mac-tombstone-conflict-rollback-\(candidate.action.actionID)",
        ...
        do {
            ...
            return CanonicalTombstoneConflictRollbackExecutionResult(
                ...
                reason: result.succeeded ? "macTombstoneConflictRollbackCompleted" : "macTombstoneConflictRollbackFailed",
            ...
            return CanonicalTombstoneConflictRollbackExecutionResult(
                ...
                succeeded: false,
                fatal: true,
                reason: "macTombstoneConflictRollbackFailed"
            )
        }
    }

    private func prepareRootBoundPayload(_ candidate: CanonicalTombstoneConflictCandidate) async throws {
        if let port = applyPort as? MacTombstoneConflictRealApplyPort {
