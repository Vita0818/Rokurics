...
struct MacCanonicalProductionTransportPort: CanonicalProductionTransportPort {
    ...
    init() {
        self.init(
            mode: .disabled,
            signerDescription: "SecureLocalHTTPSServer and RequestVerifier boundary only; production send suppressed",
            fakeSignature: nil,
            fakeResponder: nil
        )
    }
    ...
    init(
        fakeSignature: String = "mac-production-adapter-test-signature",
        fakeResponder: @escaping FakeResponder
    ) {
        self.init(
            mode: .fakeLoopback,
            signerDescription: "RequestVerifier fake verifier; no listener",
    ...
    func sendRequest(_ request: CanonicalProductionSignedRequest) async throws -> CanonicalProductionTransportExchangeResult {
        guard let fakeResponder else {
            throw CanonicalProductionPortError.networkExecutionSuppressed("macProductionTransportSendSuppressed")
        }
        ...
        guard response.hasValidBodyHash else {
            throw CanonicalTransportRuntimeError.invalidBodyHash("mac-production-fake-response")
        }
        return CanonicalProductionTransportExchangeResult(
            ...
            sideEffect: CanonicalProductionSideEffect(
                ...
                summary: "macFakeTransport:\(request.buildRequest.route.rawValue)"
            )
        )
    }
    ...
    func receiveResponse(
        ...
        guard response.hasValidBodyHash else {
            throw CanonicalTransportRuntimeError.invalidBodyHash("mac-production-response")
        }
        return CanonicalProductionTransportExchangeResult(
            request: request,
            response: response,
    ...
    func buildEnvelopeDryRun(
        ...
        return CanonicalProductionTransportEnvelopeDryRun(
            ...
            reason: isDryRunOnly ? "macProductionTransportDisabled" : "macFakeTransportProjected"
        )
    }

    func decodeResponseDryRun(_ response: CanonicalTransportResponse) async throws -> CanonicalTransportResponse {
        guard response.hasValidBodyHash else {
            throw CanonicalTransportRuntimeError.invalidBodyHash("mac-production-dry-run-response")
        }
        return response
    }
    ...
    private func validateExistingRoutePath(_ path: String, route: CanonicalTransportRoute) throws {
        ...
        guard let allowed = Self.existingRoutePaths[route], sanitized == allowed else {
            throw CanonicalProductionPortError.routeBypassRisk("macRouteNotMapped:\(route.rawValue)")
        }
    }
