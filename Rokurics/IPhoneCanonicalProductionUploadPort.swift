...
actor IPhoneCanonicalProductionUploadPort: CanonicalProductionUploadPort {
    ...
    nonisolated func classifyUploadFailure(_ failure: CanonicalProductionUploadFailure) -> CanonicalProductionUploadFailureClassification {
        ...
        if code.contains("conflict") || code == "409" {
            return CanonicalProductionUploadFailureClassification(kind: .conflict, retry: nil, reason: "iphoneUploadConflict")
        }
    ...
    nonisolated func projectUploadDryRun(
        ...
        guard artifact.kind == .audio else {
            throw CanonicalProductionPortError.unsupportedObject("iphoneUploadOnlySupportsAudio")
        }
        return CanonicalProductionUploadTrace(
            ...
            resumable: true,
            route: .uploadStart,
            reason: isDryRunOnly ? "iphoneProductionUploadDisabled" : "iphoneTestOnlyUploadProjected"
        )
    }
    ...
    private func requireTestOnlyInMemoryLedger() throws {
        guard mode == .testOnlyInMemoryLedger else {
            throw CanonicalProductionPortError.productionMutationAttempted("iphoneProductionUploadDisabled")
        }
    }
    ...
    private func sessionID(for request: CanonicalUploadStartRequest) -> CanonicalUploadSessionID {
        CanonicalUploadSessionID("iphone-\(request.objectID)-\(String(request.totalHash.value.prefix(12)))")
    }
