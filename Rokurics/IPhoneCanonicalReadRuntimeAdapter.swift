//
//  IPhoneCanonicalReadRuntimeAdapter.swift
//  Rokurics
//
...
protocol IPhoneRecordingMetadataReadSideSeamReading: Sendable {
...
extension IPhoneRecordingMetadataReadSideSeam: IPhoneRecordingMetadataReadSideSeamReading {}

struct IPhoneCanonicalReadRuntimeAdapter {
    var configuration: CanonicalReadRuntimeConfiguration
    var recordingMetadataReadSideSeam: any IPhoneRecordingMetadataReadSideSeamReading

    nonisolated init(
        configuration: CanonicalReadRuntimeConfiguration = .disabled,
        recordingMetadataReadSideSeam: (any IPhoneRecordingMetadataReadSideSeamReading)? = nil
    ) {
        self.configuration = configuration
        self.recordingMetadataReadSideSeam = recordingMetadataReadSideSeam
            ?? IPhoneRecordingMetadataReadSideSeam(configuration: configuration)
    }
    ...
    ) -> IPhoneCanonicalReadRuntimeAdapter {
        IPhoneCanonicalReadRuntimeAdapter(configuration: result.effectiveConfiguration.readRuntimeConfiguration)
    }
    ...
    nonisolated func read(
        ...
        let legacyManifest = legacyInventory.canonicalManifest
            ...
            ?? Self.makeCanonicalManifest(
                ...
                libraryObjects: legacyInventory.studyManifest.map { IPhoneCanonicalLibraryAdapter().makeLibraryObjects(from: $0) } ?? []
            )
        return read(
    ...
    nonisolated static func makeCanonicalManifest(_ manifest: StudyLibrarySyncManifest) -> CanonicalManifest {
        makeCanonicalManifest(
            ...
            libraryObjects: IPhoneCanonicalLibraryAdapter().makeLibraryObjects(from: manifest)
        )
    }
