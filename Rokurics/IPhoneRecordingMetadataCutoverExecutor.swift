...
actor IPhoneRecordingMetadataCutoverExecutor: CanonicalRecordingMetadataCutoverExecutor {
    ...
    init(
        ...
        sourceNode: CanonicalNode = CanonicalNode(nodeID: "iphone-cutover-local", platform: "iPhone", capabilities: [.recordingMetadata]),
        destinationNode: CanonicalNode = CanonicalNode(nodeID: "mac-cutover-peer", platform: "Mac", capabilities: [.recordingMetadata]),
        failureInjection: CanonicalRecordingMetadataCommitFailureInjection = .none
    ) {
        self.applyPort = ports.apply ?? IPhoneCanonicalProductionApplyPort()
    ...
    func commitRecordingMetadata(
        ...
        guard !applyPort.isDryRunOnly else {
            return .failure(candidate: candidate, kind: .applyFailureBeforeCommit, reason: "iphoneRecordingMetadataCommitRequiresInternalFakeApplyPort")
        }
        if let realApplyPort = applyPort as? IPhoneRecordingMetadataRealApplyPort {
            do {
                try await realApplyPort.prepare(candidate: candidate)
            } catch {
                return .failure(candidate: candidate, kind: .applyFailureBeforeCommit, reason: "iphoneRecordingMetadataRealApplyPortPrepareFailed")
            }
        }
        ...
        if candidate.requiresNetworkSend {
            ...
            do {
                ...
                return .failure(candidate: candidate, kind: .transportFailureBeforeSend, reason: "iphoneApplyMetadataRouteProjectionFailed")
            }
        ...
        do {
            ...
            return .failure(candidate: candidate, kind: .applyFailureBeforeCommit, reason: "iphoneRecordingMetadataApplyPortFailed")
        }
        ...
        return CanonicalRecordingMetadataProductionCommitResult(
            ...
            sideEffect: sideEffects.first,
            sideEffects: sideEffects,
            reason: candidate.requiresNetworkSend ? "iphoneRecordingMetadataSendCommitted" : "iphoneRecordingMetadataApplyCommitted"
        )
    }
    ...
    func rollbackRecordingMetadata(
        ...
        let action = CanonicalRollbackAction(
            actionID: "iphone-recording-metadata-rollback-\(candidate.action.actionID)",
        ...
        if reason == .transportFailureBeforeSend
            ...
            return CanonicalRecordingMetadataRollbackExecutionResult(
                checkpointID: candidate.effectiveRollbackCheckpointID,
                succeeded: true,
                reason: "iphoneRecordingMetadataRollbackNoOp",
        ...
        guard checkpointIDs.contains(candidate.effectiveRollbackCheckpointID) else {
            ...
            return CanonicalRecordingMetadataRollbackExecutionResult(
                ...
                succeeded: false,
                fatal: true,
                reason: "iphoneRecordingMetadataRollbackCheckpointMissing",
                rollbackResult: CanonicalRollbackResult(
                    planID: candidate.effectiveRollbackCheckpointID,
                    succeeded: false,
                    completedActionIDs: []
        ...
        do {
            ...
            return CanonicalRecordingMetadataRollbackExecutionResult(
                ...
                reason: result.succeeded ? "iphoneRecordingMetadataRollbackCompleted" : "iphoneRecordingMetadataRollbackFailed",
            ...
            return CanonicalRecordingMetadataRollbackExecutionResult(
                ...
                succeeded: false,
                fatal: true,
                reason: "iphoneRecordingMetadataRollbackFailed"
            )
        }
    }
    ...
    private func sendRouteProjection(
        ...
        guard let transportPort else {
            throw CanonicalProductionPortError.networkExecutionSuppressed("iphoneRecordingMetadataTransportPortMissing")
        }
    ...
    private func makePrecondition(
        ...
        let accepted = preconditionFailures(candidate).isEmpty
        return CanonicalProductionApplyPrecondition(
            ...
            accepted: accepted,
            reason: accepted ? "preconditionsAccepted" : preconditionFailures(candidate).joined(separator: ",")
        )
    }
