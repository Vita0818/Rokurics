...
actor IPhoneLibraryMetadataRealApplyPort: CanonicalProductionApplyPort {
    ...
    init(
        testRootURL: URL,
        rootToken: CanonicalRootToken = CanonicalRootToken("iphone-library-metadata-test-root"),
    ...
    init(
        ...
        rootToken: CanonicalRootToken = CanonicalRootToken("iphone-library-metadata-production-root"),
    ...
    func applyMetadata(_ request: CanonicalProductionApplyExecutionRequest) async throws -> CanonicalProductionApplyResult {
        try await writeMetadata(request, status: .applied, summary: "iphoneRootBoundLibraryMetadataApply")
    }

    func sendMetadata(_ request: CanonicalProductionApplyExecutionRequest) async throws -> CanonicalProductionApplyResult {
        try await writeMetadata(request, status: .sent, summary: "iphoneRootBoundLibraryMetadataSendNoNetwork")
    }
    ...
    nonisolated func verifyPrecondition(_ precondition: CanonicalProductionApplyPrecondition) async throws -> CanonicalProductionApplyPrecondition {
        if failureInjection == .preconditionMismatch {
            ...
            rejected.reason = "iphoneLibraryMetadataPreconditionMismatch"
            return rejected
        }
    ...
    nonisolated func verifyPostcondition(_ postcondition: CanonicalProductionApplyPostcondition) async throws -> CanonicalProductionApplyPostcondition {
        if failureInjection == .postconditionMismatch {
            ...
            rejected.accepted = false
            rejected.reason = "iphoneLibraryMetadataPostconditionMismatch"
    ...
    func rollbackApply(_ request: CanonicalRollbackAction) async throws -> CanonicalRollbackResult {
        guard let rootBoundCore else {
            return CanonicalRollbackResult(
                ...
                succeeded: false,
                failures: [CanonicalRollbackFailure(actionID: request.actionID, reason: "iphoneLibraryMetadataRollbackDisabled")]
            )
        }
    ...
    nonisolated func projectApplyDryRun(_ action: CanonicalApplyAction) async throws -> CanonicalProductionApplyTrace {
        CanonicalProductionApplyTrace(
            ...
            reason: isDryRunOnly ? "iphoneLibraryMetadataApplyDisabled" : "iphoneLibraryMetadataApplyProjected:\(applyPortMode.rawValue)"
        )
    }
    ...
    private func writeMetadata(
        ...
        guard let rootBoundCore else {
            throw CanonicalProductionPortError.productionMutationAttempted("iphoneLibraryMetadataApplyDisabled")
        }
        guard !isDryRunOnly else {
            throw CanonicalProductionPortError.productionMutationAttempted("iphoneLibraryMetadataProductionRootDisabled")
        }
        ...
        return CanonicalProductionApplyResult(
            ...
            precondition: CanonicalProductionApplyPrecondition(
                ...
                accepted: true,
                reason: "iphoneLibraryMetadataPreconditionAccepted"
            ),
            postcondition: CanonicalProductionApplyPostcondition(
                ...
                reason: failureInjection == .postconditionMismatch ? "iphoneLibraryMetadataPostconditionMismatch" : "iphoneLibraryMetadataPostconditionAccepted"
            ),
    ...
    private func failBeforeMutationIfNeeded() throws {
        ...
        case .applyFailureBeforeCommit, .parentMissing, .cycleDetected, .resourceMoveAttempted, .unsupportedObjectKind, .conflictDetected:
            throw CanonicalProductionPortError.productionMutationAttempted("iphoneLibraryMetadataFailureBeforeMutation")
    ...
    private func failAfterMutationIfNeeded() throws {
        if failureInjection == .applyFailureAfterPartialCommit {
            throw CanonicalProductionPortError.productionMutationAttempted("iphoneLibraryMetadataFailureAfterPartialMutation")
        }
    }
