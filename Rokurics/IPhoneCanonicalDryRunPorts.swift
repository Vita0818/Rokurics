...
nonisolated struct IPhoneCanonicalDryRunFilePort: CanonicalProductionFilePort {
    ...
    nonisolated init(rootBindings: [CanonicalRootToken: String] = [CanonicalRootToken("iphone-production-root"): "iphone/production/root"]) {
    ...
    nonisolated func projectWrite(_ intent: CanonicalFileWriteIntent) async throws -> CanonicalProductionWriteIntentProjection {
        ...
        return CanonicalProductionWriteIntentProjection(
            ...
            reason: "iPhoneDryRunWriteSuppressed"
        )
    }
}

nonisolated struct IPhoneCanonicalDryRunTransportPort: CanonicalProductionTransportPort {
    let isDryRunOnly = true
    ...
    nonisolated func buildEnvelopeDryRun(
        ...
        return CanonicalProductionTransportEnvelopeDryRun(
            ...
            reason: "iPhoneDryRunNetworkSuppressed"
        )
    }
...
nonisolated struct IPhoneCanonicalDryRunUploadPort: CanonicalProductionUploadPort {
    let isDryRunOnly = true
    ...
    nonisolated func projectUploadDryRun(
        ...
        return CanonicalProductionUploadTrace(
            ...
            resumable: true,
            route: .uploadStart,
            reason: "iPhoneDryRunUploadSuppressed"
        )
    }
}

nonisolated struct IPhoneCanonicalDryRunApplyPort: CanonicalProductionApplyPort {
    let isDryRunOnly = true
    ...
    nonisolated func projectApplyDryRun(_ action: CanonicalApplyAction) async throws -> CanonicalProductionApplyTrace {
        CanonicalProductionApplyTrace(action: action, wouldCallApplySyncManifest: false, reason: "iPhoneDryRunApplySuppressed")
    }
}

nonisolated struct IPhoneCanonicalDryRunClockPort: CanonicalProductionSyncClockPort {
...
actor IPhoneCanonicalDryRunDiagnosticsPort: CanonicalProductionDiagnosticsPort {
...
nonisolated struct IPhoneCanonicalDryRunCapabilityPort: CanonicalProductionCapabilityPort {
    private let supportedOperations: [CanonicalProductionDomain: Set<CanonicalProductionOperation>]

    nonisolated init(
        supportedOperations: [CanonicalProductionDomain: Set<CanonicalProductionOperation>] = IPhoneCanonicalDryRunCapabilityPort.defaultSupportedOperations
    ) {
...
nonisolated enum IPhoneCanonicalDryRunPorts {
    nonisolated static func makePortSet(
        rootBindings: [CanonicalRootToken: String] = [CanonicalRootToken("iphone-production-root"): "iphone/production/root"],
        chunkSizePolicy: Int = 4 * 1024 * 1024,
        diagnostics: IPhoneCanonicalDryRunDiagnosticsPort = IPhoneCanonicalDryRunDiagnosticsPort()
    ) -> CanonicalProductionPortSet {
        CanonicalProductionPortSet(
            file: IPhoneCanonicalDryRunFilePort(rootBindings: rootBindings),
            transport: IPhoneCanonicalDryRunTransportPort(),
            upload: IPhoneCanonicalDryRunUploadPort(chunkSizePolicy: chunkSizePolicy),
            apply: IPhoneCanonicalDryRunApplyPort(),
            syncClock: IPhoneCanonicalDryRunClockPort(),
            diagnostics: diagnostics,
            capability: IPhoneCanonicalDryRunCapabilityPort()
        )
    }
}
