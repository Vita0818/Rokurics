...
struct CanonicalProductionAdapterTests {
    @Test func disabledAdaptersRemainDryRunAndSuppressRealSideEffects() async throws {
        let ports = IPhoneCanonicalProductionPorts.makeDisabledPortSet()
        let readiness = ports.readiness(generatedAt: CanonicalProductionTestFixtures.date(100))
        let transport = IPhoneCanonicalProductionTransportPort()
        ...
        let uploadTrace = try await IPhoneCanonicalProductionUploadPort(chunkSizePolicy: 4 * 1024 * 1024).projectUploadDryRun(
            object: CanonicalProductionTestFixtures.recording(audio: true),
            artifact: CanonicalProductionTestFixtures.audioArtifact()
        )
        let applyTrace = try await IPhoneCanonicalProductionApplyPort().projectApplyDryRun(
        ...
        #expect(signed.signerDescription?.contains("SecureMacUploadClient") == true)
        ...
        do {
            ...
            #expect(error == .networkExecutionSuppressed("iphoneProductionTransportSendSuppressed"))
        }
    }

    @Test func filePortUsesTempRootAndRedactedResolutionTokens() async throws {
        let rootURL = try makeTempRoot("iphone-file-port")
        let file = IPhoneCanonicalProductionFilePort(testRootURL: rootURL)
    ...
    @Test func fakeTransportUploadAndApplyStayInMemory() async throws {
        let transport = IPhoneCanonicalProductionTransportPort(fakeResponder: { _ in
        ...
        let upload = IPhoneCanonicalProductionUploadPort(testOnlyChunkSizePolicy: 8)
        ...
        let start = try await upload.startResumableUpload(
            CanonicalUploadStartRequest(
                ...
                targetReference: CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "audio/recording-01.m4a"),
