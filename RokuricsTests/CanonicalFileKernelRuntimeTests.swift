...
struct CanonicalFileKernelRuntimeTests {
    ...
    @Test func fileSnapshotBuildsOffMainAndRejectsUnsafeTokens() async throws {
        ...
        let scope = try CanonicalFileSnapshotScope(
            rootToken: CanonicalRootToken("iphone-test-root"),
            logicalScopeToken: ".",
    ...
    @Test func checksumCacheCorruptionRebuildsAndDoesNotBlockFallback() async throws {
        let runtime = CanonicalFileChecksumRuntime()
        let key = CanonicalFileChecksumCacheKey(
            rootToken: CanonicalRootToken("checksum-root"),
            logicalToken: "recordings/audio.m4a",
            byteSize: 10,
            modifiedAtEpochMs: 1_000,
            contentVersion: "v1",
            schemaVersion: "canonical-file-checksum-v910",
            algorithm: "sha256",
            domainHint: .recordingAudio
        )
        await runtime.storeForTesting(
            CanonicalFileChecksumCacheRecord(
                key: key,
                fullHash: "corrupt",
                hashPrefix: "corrupt",
                computedAt: CanonicalTimestamp(Date(timeIntervalSince1970: 2_000)),
                validationState: .corrupted
            )
        )
        let request = try CanonicalFileChecksumRequest(
            rootToken: key.rootToken,
            logicalToken: key.logicalToken,
            byteSize: key.byteSize,
            modifiedAt: Date(timeIntervalSince1970: 1),
            contentVersion: key.contentVersion,
            domainHint: key.domainHint
        ) {
            String(repeating: "e", count: 64)
        }
        let result = await runtime.checksum(request)

        #expect(result.event == .miss)
        #expect(result.failure == .cacheCorrupted)
        #expect(result.validationState == .valid)
        #expect(result.hashPrefix == String(repeating: "e", count: 12))
    }
    ...
    @Test func asyncDiagnosticsWriterRejectsUnsafeDetailsAndUsesClock() async {
        ...
        let unsafe = CanonicalKernelDiagnosticRecord(
            kind: .diagnosticsWriteDurationMs,
            domain: .file,
            redactedDetail: "requestBody=/Users/vita/private.json"
        )
        ...
        #expect(!lines.joined().contains("requestBody"))
    }

    @Test func asyncDiagnosticsWriterBackpressureDropsNewestWhenQueueFull() async {
        let sink = CanonicalInMemoryDiagnosticsSink()
        let writer = CanonicalAsyncDiagnosticsWriter(
            sink: sink,
            configuration: CanonicalAsyncDiagnosticsWriterConfiguration(maxQueuedEvents: 1, automaticallyFlush: false)
        )
        let first = CanonicalKernelDiagnosticRecord(
            kind: .diagnosticsWriteDurationMs,
            domain: .file,
            redactedDetail: "cacheKeyPrefix=first"
        )
        let second = CanonicalKernelDiagnosticRecord(
            kind: .diagnosticsWriteDurationMs,
            domain: .file,
            redactedDetail: "cacheKeyPrefix=second"
        )

        #expect(await writer.enqueue(first) == .enqueued)
        #expect(await writer.enqueue(second) == .droppedBackpressure)
        await writer.drainForTests()
        let metrics = await writer.currentMetrics()
        let lines = await sink.allLines()

        #expect(metrics.enqueuedCount == 1)
        #expect(metrics.droppedCount == 1)
        #expect(metrics.writtenCount == 1)
        #expect(lines.joined().contains("first"))
        #expect(lines.joined().contains("second") == false)
    }
    ...
    @Test func connectionDiagnosticsRecordIsMainActorNonBlockingAndFlushesRedactedJSONL() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("ConnectionDiagnosticsAsync-\(UUID().uuidString)", isDirectory: true)
        ...
        for index in 0..<1_000 {
            store.record(
                ...
                deviceID: "iphone-diagnostics-device",
                result: index == 999 ? "request body /Users/vita/private.json hash=\(fullHash)" : "safe-index-\(index)",
        ...
        #expect(raw.contains("request body") == false)
    ...
    @Test func connectionDiagnosticsNoiseStormPersistsCriticalTerminalsInOrder() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("ConnectionDiagnosticsCriticalStorm-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = ConnectionDiagnosticsStore(
            rootURL: root,
            maxEntries: 10,
        ...
        for index in 0..<100 {
            store.record(phase: "noiseBefore\(index)", result: "normal")
        }
        store.record(phase: "syncRunCompleted", syncRunID: "critical-run")
        store.record(phase: "syncTickFailed", syncRunID: "critical-run")
        store.record(phase: "uploadFinalizeFailed", syncRunID: "critical-run")
        store.record(phase: "metadataApplyFailed", syncRunID: "critical-run")
        store.record(
        ...
        for index in 0..<100 {
            store.record(phase: "noiseAfter\(index)", result: "normal")
        }
    ...
    @Test func mainActorHotPathGuardCountsInjectedAttempts() async {
        ...
        await guardOff.recordAttempt(kind: .fullHash, reason: "fullHash")
    ...
    @Test func readinessV910ReportsNoFreezeScorecardAndUnsafeBlockers() {
        ...
        #expect(ready.noFreezeScorecard.fileTreeOffMainReady)
        #expect(ready.noFreezeScorecard.routeSecurityUnchanged)
        #expect(unsafe.status == .unsafeToProceed)
        #expect(unsafe.blockers.contains(.routeSecurityChanged))
        #expect(unsafe.blockers.contains(.defaultReleaseCanonicalEnabled))
