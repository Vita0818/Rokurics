...
struct CanonicalKernelFacadeTests {
    ...
    @Test func dryRunModeReturnsDryRunReportOnly() {
        let facade = CanonicalKernelFacade(
            ...
            environment: CanonicalKernelEnvironment(ports: IPhoneCanonicalDryRunPorts.makePortSet())
        )
        let local = CanonicalProductionTestFixtures.snapshot()
        let peer = CanonicalProductionTestFixtures.snapshot(node: CanonicalProductionTestFixtures.node("mac-01", platform: "Mac"))
