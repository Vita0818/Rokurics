...
struct CanonicalRecordingMetadataProductionExecutionTests {
    @Test func iPhoneFacadeDefaultCutoverIsDisabledAndDoesNotCommit() async {
        let executor = RecordingMetadataCutoverTestSupport.FakeExecutor()
        let result = await CanonicalIPhoneMigrationFacade().runRecordingMetadataCutover(
    ...
    @Test func sendCandidateUsesExistingApplyMetadataRouteAndNoUploadSideEffects() async throws {
        ...
        #expect(IPhoneCanonicalProductionTransportPort().existingRoutePath(for: .applyMetadata) == "/sync/apply-metadata")
