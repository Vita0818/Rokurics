...
struct CanonicalRecordingMetadataCommitExecutorTests {
    @Test func defaultExecutorBlocksProductionCommitWithoutInternalFakePort() async {
        let executor = IPhoneRecordingMetadataCutoverExecutor()
    ...
    @Test func applyCommitUsesApplyPortAndRecordsSafeMetadataSideEffect() async throws {
        ...
        let executor = IPhoneRecordingMetadataCutoverExecutor(applyPort: apply)
    ...
    @Test func sendCommitUsesApplyMetadataRouteProjectionAndSendMetadataPort() async throws {
        ...
        let transport = IPhoneCanonicalProductionTransportPort { request in
        ...
        let executor = IPhoneRecordingMetadataCutoverExecutor(applyPort: apply, transportPort: transport)
    ...
    @Test func partialCommitFailureRollsBackAndRollbackFailureIsFatal() async {
        let rollbackOK = IPhoneRecordingMetadataCutoverExecutor(
        ...
        let rollbackFails = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: CommitTrackingApplyPort(),
    ...
    @Test func preconditionMismatchAndTransportFailureBlockCommitBeforeSuppression() async {
        let preconditionExecutor = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: CommitTrackingApplyPort(),
        ...
        let transportExecutor = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: CommitTrackingApplyPort(),
            transportPort: IPhoneCanonicalProductionTransportPort { _ in CanonicalTransportResponse(ok: true, status: "ok") },
        ...
        let apply = CommitTrackingApplyPort()
        let applyBeforeExecutor = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: apply,
    ...
    @Test func nonRecordingMetadataCandidateAndUnexpectedSideEffectsAreBlocked() async {
        let generated = await Self.run(
            ...
            executor: IPhoneRecordingMetadataCutoverExecutor(applyPort: CommitTrackingApplyPort())
        )
        ...
        let unexpectedSideEffect = await Self.run(
            configuration: .canary(maxObjects: 1, allowsV87CanaryN1InternalExecution: true),
            executor: IPhoneRecordingMetadataCutoverExecutor(applyPort: CommitTrackingApplyPort(unexpectedSideEffect: true))
        )
    ...
    @Test func fakeInMemoryPortRollsBackPartialMutationAndLeavesUnrelatedObjectsUntouched() async {
        let apply = IPhoneCanonicalProductionApplyPort(fakeInMemory: true)
        ...
        let stableCommit = await IPhoneRecordingMetadataCutoverExecutor(applyPort: apply).commitRecordingMetadata(stable)
        let partialExecutor = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: apply,
            failureInjection: .applyFailureAfterPartialCommit
        )
    ...
    @Test func duplicateObjectDifferentActionIDIsNotSkippedAndFailureReplayIsNotSuccess() async {
        ...
        let failing = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: CommitTrackingApplyPort(),
            failureInjection: .preconditionMismatch
        )
    ...
    @Test func acceptedTransportMissingCheckpointAndForbiddenSideEffectsRollbackSafely() async {
        let transportAccepted = IPhoneRecordingMetadataCutoverExecutor(
            applyPort: CommitTrackingApplyPort(),
            transportPort: IPhoneCanonicalProductionTransportPort { _ in CanonicalTransportResponse(ok: true, status: "ok") },
        ...
        let missingCheckpoint = await Self.run(
            configuration: .canary(maxObjects: 1, allowsV87CanaryN1InternalExecution: true),
            executor: IPhoneRecordingMetadataCutoverExecutor(
                applyPort: CommitTrackingApplyPort(),
        ...
        let forbiddenSideEffect = await Self.run(
            configuration: .canary(maxObjects: 1, allowsV87CanaryN1InternalExecution: true),
            executor: IPhoneRecordingMetadataCutoverExecutor(
                applyPort: IPhoneCanonicalProductionApplyPort(fakeInMemory: true, failureInjection: .unsupportedSideEffect)
            )
        )
