...
struct CanonicalProductionPortsTests {
    @Test func dryRunPortSetDeclaresRequiredPortsAndCapabilities() {
        let ports = IPhoneCanonicalDryRunPorts.makePortSet()
        let readiness = ports.readiness(generatedAt: CanonicalProductionTestFixtures.date(10))
        let summary = IPhoneCanonicalDryRunCapabilityPort().summary(for: CanonicalProductionTestFixtures.node())
    ...
    @Test func fileTransportUploadAndApplyPortsOnlyProjectSuppressedWork() async throws {
        let root = CanonicalRootToken("iphone-production-root")
        let file = IPhoneCanonicalDryRunFilePort(rootBindings: [root: "iphone/production/root"])
        ...
        let transport = IPhoneCanonicalDryRunTransportPort()
        let envelope = try await transport.buildEnvelopeDryRun(
            source: CanonicalProductionTestFixtures.node("iphone-01"),
        ...
        let upload = try await IPhoneCanonicalDryRunUploadPort().projectUploadDryRun(
            object: object,
        ...
        let apply = try await IPhoneCanonicalDryRunApplyPort().projectApplyDryRun(action)
    ...
    @Test func productionFilePortDefaultAndReleaseRemainBlocked() async throws {
        let rootURL = try makeTempRoot("iphone-file-release")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let reference = CanonicalFileReference(rootToken: CanonicalRootToken("iphone-production-root"), logicalPathToken: "metadata/recording-01.json")
        let intent = makeIntent(reference: reference, bytes: Data("blocked".utf8), purpose: .metadataBlob)
        let defaultPort = IPhoneCanonicalProductionFilePort()
        ...
        let releasePort = IPhoneCanonicalProductionFilePort(productionRootURL: rootURL, policy: releasePolicy)
    ...
    @Test func productionFilePortTestRootTrueWriteIsRootBoundAtomicAndRedacted() async throws {
        let rootURL = try makeTempRoot("iphone-file-test-root")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let port = IPhoneCanonicalProductionFilePort(testRootURL: rootURL)
        let reference = CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "metadata/recording-01.json")
        ...
        #expect(write.evidence.resolution.resolvedPathToken == "iphone-test-root/metadata/recording-01.json")
    ...
    @Test func productionFilePortProductionRootRequiresConfirmationAndTestHarnessClone() async throws {
        let rootURL = try makeTempRoot("iphone-file-production-gate")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let reference = CanonicalFileReference(rootToken: CanonicalRootToken("iphone-production-root"), logicalPathToken: "metadata/recording-01.json")
        let intent = makeIntent(reference: reference, bytes: Data("blocked".utf8), purpose: .metadataBlob)

        let missingConfirmation = IPhoneCanonicalProductionFilePort(
        ...
        let missingHarness = IPhoneCanonicalProductionFilePort(
            productionRootURL: rootURL,
            policy: .explicitProductionRoot(
                explicitDebugInternalConfiguration: true,
                ownerApproved: true,
        ...
        let allowed = IPhoneCanonicalProductionFilePort(
            productionRootURL: rootURL,
            policy: .explicitProductionRoot(
                explicitDebugInternalConfiguration: true,
                ownerApproved: true,
    ...
    @Test func productionFilePortPathEscapeIsBlocked() async throws {
        let rootURL = try makeTempRoot("iphone-file-escape")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let port = IPhoneCanonicalProductionFilePort(testRootURL: rootURL)

        do {
            _ = try await port.resolveLogicalToken("../escape.json", rootToken: CanonicalRootToken("iphone-test-root"))
    ...
    @Test func productionFilePortRollbackRestoresOldBytesAndDeletesNewFile() async throws {
        let rootURL = try makeTempRoot("iphone-file-rollback")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let port = IPhoneCanonicalProductionFilePort(testRootURL: rootURL)
        let reference = CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "metadata/recording-01.json")
        ...
        let newReference = CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "metadata/recording-02.json")
        _ = try await port.writeMetadata(
    ...
    @Test func productionFilePortPostconditionAndDomainRoutesAreVerified() async throws {
        let rootURL = try makeTempRoot("iphone-file-domains")
        defer { try? FileManager.default.removeItem(at: rootURL) }
        let port = IPhoneCanonicalProductionFilePort(testRootURL: rootURL)
        let routes: [(CanonicalFileReference, CanonicalFilePurpose)] = [
            (CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "recordingMetadata/recording-01.json"), .metadataBlob),
            (CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "libraryMetadata/folders/folder-01.json"), .metadataBlob),
            (CanonicalFileReference(rootToken: CanonicalRootToken("iphone-test-root"), logicalPathToken: "generatedArtifacts/recording-01/note.md", artifactKind: .noteMarkdown), .generatedArtifact)
        ]
    ...
    private func expectProductionWriteBlocked(_ port: IPhoneCanonicalProductionFilePort, intent: CanonicalFileWriteIntent) async {
        do {
