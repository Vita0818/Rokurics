...
struct CanonicalMigrationMatrixTests {
    ...
    private func v814LibraryConfiguration() -> CanonicalLibraryMetadataCutoverAppSeamConfiguration {
        .enabled(
            ...
            evidence: CanonicalLibraryMetadataCutoverEvidence.passing(
                rollbackPlan: CanonicalRollbackPlan(planID: "v814-matrix-rollback", checkpoints: [], actions: [])
            ),
            cutoverToken: CanonicalCutoverToken(tokenID: "v814-matrix-token", syncRunID: "v814-matrix", ownerApproved: true)
        )
    }
    ...
    private func v814LibraryContext(
        ...
        let manifest = CanonicalManifest.make(
            node: CanonicalNode(nodeID: "matrix-node", platform: "test", capabilities: [.canonicalLibraryObjectsV1]),
